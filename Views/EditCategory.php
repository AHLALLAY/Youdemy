...
if (isset($_POST['update_category'])) {
    ...
    if ($category->updateCategory()) {
        ...
        echo "<script>alert('Échec de la mise à jour de la catégorie.')</script>";
    }
}
...
<head>
    ...
    <title>Youdemy - Modifier Catégorie</title>
</head>
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
                    <h3 class="text-xl font-semibold text-gray-800">Modifier la Catégorie</h3>
                </div>

                <form method="post" class="space-y-6">
                    <input type="hidden" name="category_id" value="<?= htmlspecialchars($category_data['category_id']) ?>">

                    <div class="space-y-2">
                        <label for="new_name" class="block text-sm font-medium text-gray-700">Nom de la catégorie</label>
                        <input type="text"id="new_name"name="new_name"
                            value="<?= htmlspecialchars($category_data['category_name']) ?>"
