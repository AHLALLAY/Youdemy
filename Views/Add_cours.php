...
$category = $categories->display('category');
    ...
    if ($_FILES['course_content']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes) && $fileSize <= $maxFileSize) {
            ...
            if (move_uploaded_file($fileTmpPath, $fileDest)) {
                ...
                $new_cours = new Cours(null,$title, $description, $fileDest, null, $_SESSION['id'], $cate);
                $new_cours->addCours();
                ...
                <div class="p-6">
                    <form method="post" enctype="multipart/form-data" class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="course_title" id="course_title" placeholder="Entrez le titre du cours" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all duration-300 text-gray-600 placeholder-gray-400">
                        </div>
                        <div class="flex space-x-4">
                            <div class="space-y-2">
                                <label for="course_description" class="block text-sm font-medium text-gray-700">Description du cours</label>
                                <textarea name="course_description" id="course_description" placeholder="Entrez la description du cours" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all duration-300 text-gray-600 placeholder-gray-400"></textarea>
                            </div>
                            <div class="space-y-2">
                                <label for="course_content" class="block text-sm font-medium text-gray-700">Contenu du cours</label>
                                <input type="file" name="course_content" id="course_content" placeholder="Entrez le contenu du cours" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all duration-300 text-gray-600 placeholder-gray-400"></textarea>
                            </div>
                        </div>
                        <div class="space-y-2">
                            <label for="course_category" class="block text-sm font-medium text-gray-700">categorie du cours</label>
                            <select name="course_category" id="course_category" placeholder="Entrez le contenu du cours" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all duration-300 text-gray-600 placeholder-gray-400">
                                <option value="choix">Selectionner ... </option>
                        ...
                        <div class="flex justify-end space-x-4">
                            ...
                            <button type="submit" name="add_course" class="px-6 py-2.5 rounded-xl bg-blue-500 text-white hover:bg-blue-600 transition-all duration-300">
                                Ajouter
                            </button>
                        </div>
                        <div>
                            <?php
                                if(!empty($cat) && !empty($title) && !empty($description) && !empty($fileDest)){
                                    echo $cat . '<br>';
                                    echo $title . '<br>';
                                    echo $description . '<br>';
                                    echo $fileDest . '<br>';
